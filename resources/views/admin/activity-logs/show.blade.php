...
@section('title', 'Detail Activity Log - PT. Sinom Jati Mas')
...
<div class="max-w-4xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
            ...
            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-{{ $activityLog->action_color }}-100 text-{{ $activityLog->action_color }}-800">
        ...
        <div class="p-6">
            <div class="mb-6">
                <h2 class="text-2xl font-bold text-gray-900">{{ $activityLog->description }}</h2>
                ...
                <div>
                    ...
                    <dl class="space-y-3">
                        <div class="flex justify-between">
                            ...
                            <dd class="text-sm font-medium text-gray-900">{{ $activityLog->user?->name ?? 'System' }}</dd>
                        </div>
                        <div class="flex justify-between">
                        ...
                        <div class="flex justify-between">
                            ...
                            <dd class="text-sm text-gray-900">{{ $activityLog->user?->role ?? '-' }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-sm text-gray-500">IP Address</dt>
                            <dd class="text-sm font-mono text-gray-900">{{ $activityLog->ip_address }}</dd>
                        </div>
                    </dl>
                </div>
                ...
                <div>
                    ...
                    <dl class="space-y-3">
                        <div class="flex justify-between">
                            ...
                            <dd class="text-sm font-medium text-gray-900">{{ class_basename($activityLog->model_type) }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-sm text-gray-500">ID</dt>
                            <dd class="text-sm font-mono text-gray-900">{{ $activityLog->model_id }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
        ...
        <div class="grid md:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-6">
                        <div class="bg-gray-50 rounded-lg p-4 overflow-x-auto">
                            <pre class="text-xs text-gray-700">{{ json_encode($activityLog->old_values, JSON_PRETTY_PRINT) }}</pre>
                        </div>
                    </div>
                </div>
            @endif
            ...
            @if($activityLog->new_values)
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-6">
                        <div class="bg-gray-50 rounded-lg p-4 overflow-x-auto">
                            <pre class="text-xs text-gray-700">{{ json_encode($activityLog->new_values, JSON_PRETTY_PRINT) }}</pre>
                        </div>
                    </div>
                </div>
            @endif
    ...
    <div class="flex justify-between">
        ...
        <button type="button"
                ...
                @click="$dispatch('open-confirm', {
                    ...
                    message: 'Yakin ingin menghapus log aktivitas ini? Tindakan ini tidak dapat dibatalkan.',
                    action: '{{ route('admin.activity-logs.destroy', $activityLog) }}',
                    method: 'DELETE',
                    buttonText: 'Hapus Log',
