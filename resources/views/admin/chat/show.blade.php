...
@section('content')
<div class="h-[calc(100vh-180px)] flex flex-col bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden animate-fade-in" x-data="{ showConvertModal: false, converting: false }">
    {{-- Chat Header --}}
    <div class="p-4 border-b border-gray-200 flex items-center justify-between bg-gradient-to-r from-primary-50 to-secondary-50">
        <div class="flex items-center space-x-3">
            <div class="w-12 h-12 rounded-full overflow-hidden border-2 border-white shadow-md">
                <img src="{{ $conversation->user->photo_url }}" alt="{{ $conversation->user->name }}" class="w-full h-full object-cover">
            </div>
            <div>
                <h3 class="font-semibold text-gray-900">{{ $conversation->user->name }}</h3>
                <p class="text-xs text-gray-500 flex items-center">
                    <span class="w-2 h-2 bg-green-500 rounded-full mr-2 animate-pulse"></span>
                    {{ $conversation->user->email }} • Online
                </p>
            </div>
        </div>
        <div class="flex items-center gap-2">
            @if(!$conversation->isConverted())
                <button @click="showConvertModal = true" 
                        class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-primary-500 to-secondary-500 rounded-lg hover:shadow-md transition-all">
                    <i class="fa-solid fa-briefcase mr-2"></i>
                    Buat Project
                </button>
            @else
                <a href="{{ route('admin.projects.show', $conversation->project) }}" 
                   class="inline-flex items-center px-4 py-2 text-sm font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-all">
    ...
    <div x-show="showConvertModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @keydown.escape.window="showConvertModal = false">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 overflow-hidden" @click.away="showConvertModal = false">
            <div class="p-6 border-b border-gray-200">
                <h3 class="text-lg font-bold text-gray-900">Buat Project dari Percakapan</h3>
                <p class="text-sm text-gray-500 mt-1">Customer: <strong>{{ $conversation->user->name }}</strong></p>
            </div>
            <form id="convert-form" class="p-6 space-y-4">
                @csrf
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Project</label>
                    <input type="text" name="name" required class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500" placeholder="Contoh: Pembangunan Rumah Tinggal">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Lokasi</label>
                    <input type="text" name="location" class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500" placeholder="Contoh: Jakarta Selatan">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nilai Kontrak (Rp)</label>
                    <input type="number" name="contract_value" min="0" class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500" placeholder="0">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Deskripsi</label>
                    <textarea name="description" rows="3" class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500" placeholder="Deskripsi project..."></textarea>
                </div>
            </form>
            <div class="p-4 bg-gray-50 flex justify-end gap-3">
                <button @click="showConvertModal = false" class="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-800">Batal</button>
                <button @click="submitConvert()" :disabled="converting" class="px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-primary-500 to-secondary-500 rounded-lg hover:shadow-md disabled:opacity-50">
                    <span x-show="!converting">Buat Project</span>
                    <span x-show="converting"><i class="fa-solid fa-circle-notch fa-spin mr-1"></i> Memproses...</span>
                </button>
            </div>
        </div>
    </div>
...
<script>
    ...
    async function submitConvert() {
        ...
        Alpine.store('converting', true);
        
        try {
            const res = await fetch(`/admin/conversations/{{ $conversation->id }}/convert`, {
                method: 'POST',
                body: formData,
                headers: {
                    'X-CSRF-TOKEN': csrfToken,
