...
@section('content')
<div class="max-w-5xl mx-auto animate-fade-in" x-data="invoiceForm()">
    <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
        
        {{-- Header Form --}}
        <div class="px-8 py-5 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
            <h3 class="text-xl font-black text-gray-900 tracking-tight">Form Invoice Baru</h3>
            <span class="bg-blue-100 text-blue-700 text-xs font-bold px-3 py-1.5 rounded-full shadow-sm flex items-center">
                <i class="fa-solid fa-paper-plane mr-1.5"></i> Auto-Send ke Chat Klien
            </span>
        </div>
        
        <div class="p-8">
            <form action="{{ route('admin.invoices.store') }}" method="POST" @submit="loading = true">
                @csrf
                
                {{-- Bagian Atas: Klien, Proyek, Termin & Jatuh Tempo --}}
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
                    
                    {{-- 1. Dropdown Klien --}}
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Klien <span class="text-red-500">*</span></label>
                        <select name="client_id" x-model="selectedClient" @change="selectedProject = ''" class="w-full border-gray-200 bg-gray-50 rounded-xl focus:ring-[#DD3517] focus:border-[#DD3517] font-medium text-sm transition-colors" required>
                    ...
                    {{-- 2. Dropdown Proyek (Dinamis filter by Client) --}}
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Proyek <span class="text-red-500">*</span></label>
                        <select name="project_id" x-model="selectedProject" class="w-full border-gray-200 bg-gray-50 rounded-xl focus:ring-[#DD3517] focus:border-[#DD3517] disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed font-medium text-sm transition-colors" :disabled="!selectedClient" required>
                        ...
                        <div class="relative" @click.away="terminOpen = false">
                            <div class="relative flex items-center">
                                ...
                                <input type="number" name="termin_percentage" x-model="terminPercentage" @focus="terminOpen = true" 
                                       class="block w-full pl-10 pr-16 py-2.5 bg-gray-50 border-gray-200 rounded-xl focus:ring-[#DD3517] focus:border-[#DD3517] font-medium text-sm transition-colors" 
                                       placeholder="Angka..." required min="1" max="100">
                    ...
                    <div>
                        ...
                        <input type="date" name="due_date" class="w-full border-gray-200 bg-gray-50 rounded-xl focus:ring-[#DD3517] focus:border-[#DD3517] font-medium text-sm transition-colors text-gray-700">
                    </div>
                </div>

                {{-- Bagian Tengah: Dynamic Invoice Items --}}
                <div class="mb-6">
                    <div class="flex justify-between items-end mb-4">
                        ...
                        <button type="button" @click="addItem()" class="text-xs bg-gray-900 text-white px-4 py-2 rounded-xl hover:bg-gray-800 transition-all shadow-md transform hover:-translate-y-0.5">
                        ...
                        <table class="w-full text-sm text-left">
                            <thead class="text-xs text-gray-600 uppercase bg-gray-100 border-b border-gray-200 font-bold">
                                <tr>
                                    ...
                                    <th class="px-4 py-4 w-32">Satuan</th>
                                    ...
                                    <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                        ...
                                        <td class="px-4 py-2">
                                            <input type="text" x-model="item.name" :name="'items['+index+'][item_name]'" required 
                                                   ...
                                                   placeholder="Contoh: Pemasangan Keramik 60x60">
                                        </td>
                                        <td class="px-4 py-2">
                                            <input type="number" step="0.1" x-model="item.qty" :name="'items['+index+'][quantity]'" required 
                                        ...
                                        <td class="px-4 py-2 text-center">
                                            <button type="button" @click="removeItem(index)" class="text-gray-400 hover:text-red-500 hover:bg-red-50 w-8 h-8 rounded-lg transition-all" title="Hapus Baris">
                ...
                <div class="flex justify-end space-x-4 pt-6 mt-6 border-t border-gray-100">
                    <a href="{{ route('admin.invoices.index') }}" class="inline-flex items-center px-6 py-3 border-2 border-gray-200 text-gray-600 text-sm font-bold rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all">
                        Batal
                    </a>
                    <button type="submit" :disabled="loading || items.length === 0" class="inline-flex items-center px-8 py-3 bg-[#DD3517] text-white text-sm font-bold rounded-xl hover:bg-[#FF812E] transition-all disabled:opacity-70 disabled:cursor-not-allowed shadow-lg shadow-orange-200 transform hover:-translate-y-1">
                        <i x-show="!loading" class="fa-solid fa-file-invoice-dollar mr-2"></i>
                        <i x-show="loading" class="fa-solid fa-circle-notch fa-spin mr-2" style="display: none;"></i>
                        <span x-text="loading ? 'Menyimpan & Mengirim...' : 'Simpan & Terbitkan Invoice'"></span>
                    </button>
                </div>
            </form>
        </div>
...
function invoiceForm() {
    return {
        ...
        // --- COMBOBOX TERMIN ---
        terminOpen: false,
        terminPercentage: 100, // Default 100%
        ...
        addItem() {
            this.items.push({ name: '', qty: 1, unit: '', price: 0 });
        },
        ...
        formatCurrency(value) {
            // Format output menjadi misal: 15.000.000
