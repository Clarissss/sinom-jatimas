    ...
    <div class="flex w-full max-w-5xl bg-white rounded-[2rem] shadow-2xl shadow-gray-300/80 overflow-hidden min-h-[600px] border border-gray-100 pointer-events-auto relative">
        ...
        <div class="w-full lg:w-1/2 flex items-center justify-center relative bg-white bg-grid-pattern overflow-y-auto px-6 py-12 lg:px-12">
            ...
            <div class="w-full max-w-md relative z-10">
                ...
                {{-- Form Elements --}}
                <form method="POST" action="{{ route('password.email') }}" class="space-y-6" x-data="{ loading: false }" @submit.prevent="loading = true; $el.submit()">
                    @csrf
                    ...
                    <div class="input-group group">
                        ...
                        <div class="relative flex items-center">
                            ...
                            <input id="email" name="email" type="email" required autofocus
                                   class="block w-full pl-14 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-2xl text-gray-900 placeholder-gray-400 focus:bg-white focus:ring-2 focus:ring-[#DD3517]/20 focus:border-[#DD3517] transition-all font-medium @error('email') border-red-500 @enderror"
                                   value="{{ old('email') }}" placeholder="admin@sinomjatimas.com">
                        </div>
                        @error('email')
                            <p class="mt-2 text-sm text-red-500 font-medium flex items-center"><i class="fa-solid fa-circle-exclamation mr-1.5"></i> {{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mt-10 text-center border-t border-gray-100 pt-6">
                    <a href="{{ route('login') }}" class="inline-flex items-center text-sm font-bold text-gray-500 hover:text-[#DD3517] transition-colors group">
