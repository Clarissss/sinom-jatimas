...
<div class="space-y-6 animate-fade-in">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-xl shadow-sm p-5 text-center border border-gray-100 hover:shadow-md transition-shadow">
            <div class="text-2xl font-bold text-secondary-600">{{ $stats['active_projects'] }}</div>
            <p class="text-xs text-gray-500 mt-1 uppercase tracking-wide">Sedang Berjalan</p>
        </div>
        <div class="bg-white rounded-xl shadow-sm p-5 text-center border border-gray-100 hover:shadow-md transition-shadow">
            <div class="text-2xl font-bold text-green-600">{{ $stats['completed_projects'] }}</div>
            <p class="text-xs text-gray-500 mt-1 uppercase tracking-wide">Selesai</p>
        </div>
        <div class="bg-white rounded-xl shadow-sm p-5 text-center border border-gray-100 hover:shadow-md transition-shadow">
            <div class="text-2xl font-bold text-yellow-600">{{ $stats['pending_invoices'] }}</div>
                ...
                <div class="flex flex-col lg:flex-row lg:items-start gap-6">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center text-gray-500 text-sm mb-4">
                            ...
                            <span class="truncate">{{ $project->location ?? 'Lokasi belum ditentukan' }}</span>
                        </div>
                        ...
                        <div class="mb-4">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span class="text-sm font-bold text-primary-600">{{ $project->progress_percentage }}%</span>
                            </div>
                            <div class="w-full bg-gray-200 rounded-full h-2.5">
                                <div class="bg-gradient-to-r from-primary-500 to-secondary-500 h-2.5 rounded-full transition-all duration-500" style="width: {{ $project->progress_percentage }}%"></div>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap gap-4 text-sm text-gray-600">
                            <div class="flex items-center">
                                ...
                                <span>{{ $project->progressPhotos->count() }} Foto</span>
                            </div>
                            <div class="flex items-center">
                                <svg class="w-4 h-4 mr-1.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                </svg>
                                <span>{{ $project->documents->count() }} Dokumen</span>
                            </div>
                            <div class="flex items-center">
                                <svg class="w-4 h-4 mr-1.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
                                </svg>
                                <span>{{ $project->invoices->count() }} Invoice</span>
                            </div>
                        </div>
                    </div>
