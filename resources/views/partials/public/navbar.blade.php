...
<header id="header" class="fixed inset-x-0 top-4 z-50 px-3 transition-all duration-300 sm:px-6">
    <div id="nav-bar" class="mx-auto flex h-16 w-full max-w-7xl items-center justify-between rounded-2xl border-0 bg-white/10 px-4 backdrop-blur-xl transition-all duration-300 sm:px-6 lg:px-8">
        <a href="{{ route('landing') }}" class="flex items-center gap-3">
            @if(!empty($companyProfile?->logo))
                <img src="{{ $companyProfile->imageUrl($companyProfile->logo) }}" alt="Logo" class="h-10 w-10 rounded-xl object-contain bg-white/90 p-1">
            @else
            ...
            @endif
            <span id="nav-brand" class="text-sm font-bold tracking-wide text-white drop-shadow-sm sm:text-base">{{ $companyProfile?->company_name ?? 'PT. SINOM JATI MAS' }}</span>
        </a>
        ...
        <div id="nav-actions" class="hidden items-center gap-3 md:flex">
            <a href="{{ route('login') }}" class="nav-btn-login rounded-full border border-white/35 bg-white/10 px-4 py-2 text-sm font-medium text-white transition hover:bg-white/20">Login</a>
            <a href="{{ route('register') }}" class="rounded-full bg-gradient-to-r from-brand to-secondary-500 px-4 py-2 text-sm font-semibold text-white shadow-md transition hover:scale-[1.03]">Register</a>
        </div>
    ...
    <div id="mobile-menu" class="mx-auto mt-3 hidden w-full max-w-7xl rounded-2xl border border-white/25 bg-gray-900/70 px-4 py-4 backdrop-blur-xl md:hidden">
        ...
        <div class="mt-4 grid grid-cols-2 gap-2">
            ...
            <a href="{{ route('register') }}" class="rounded-full bg-gradient-to-r from-brand to-secondary-500 px-3 py-2 text-center text-sm font-semibold text-white">Register</a>
        </div>
    </div>
</header>
