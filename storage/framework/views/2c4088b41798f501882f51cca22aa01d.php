<?php $__env->startSection('title', 'Chat - ' . $project->name); ?>
<?php $__env->startSection('page-title', 'Live Chat'); ?>
...
<div class="h-[calc(100vh-180px)] flex flex-col bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden animate-fade-in">
        ...
        <div class="flex items-center space-x-3">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 flex items-center">
                    ...
                    <?php echo e($project->client->name); ?> • Online
                </p>
            </div>
        </div>
        <a href="<?php echo e(auth()->user()->isAdmin() ? route('admin.projects.show', $project) : route('client.projects.show', $project)); ?>" 
    ...
    <div id="messages-container" class="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50" role="log" aria-live="polite" aria-label="Chat messages">
        <?php $__empty_1 = true; $__currentLoopData = $messages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $message): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="message-item <?php echo e($message->sender_id === auth()->id() ? 'sent' : 'received'); ?>" 
                 data-message-id="<?php echo e($message->id); ?>"
                 data-file-type="<?php echo e($message->file_type); ?>"
                 data-file-name="<?php echo e($message->file_name); ?>">
                
                <?php if($message->sender_id !== auth()->id()): ?>
                    <div class="avatar" aria-hidden="true">
                        <img src="<?php echo e($message->sender->photo_url); ?>" alt="<?php echo e($message->sender->name); ?>">
                    </div>
                <?php endif; ?>
                
                <div class="message-bubble <?php echo e($message->sender_id === auth()->id() ? 'bg-gradient text-white' : 'bg-white'); ?>">
                    <?php if($message->sender_id !== auth()->id()): ?>
                        <p class="sender-name"><?php echo e($message->sender->name); ?></p>
                    <?php endif; ?>
                    
                    <?php if($message->message): ?>
                        <p class="message-text"><?php echo nl2br(e($message->message)); ?></p>
                    <?php endif; ?>
                    ...
                    <?php if($message->file_path): ?>
                        <?php if($message->file_type === 'image'): ?>
                            ...
                            <div class="file-preview-image">
                                <a href="<?php echo e(route('chat.download', $message)); ?>" target="_blank" rel="noopener noreferrer">
                                    <img src="<?php echo e(route('chat.download', $message)); ?>" 
                                         alt="<?php echo e($message->file_name); ?>"
                                         loading="lazy">
                                </a>
                            </div>
                        <?php else: ?>
                            
                            <a href="<?php echo e(route('chat.download', $message)); ?>" class="file-download">
                                <div class="file-icon" aria-hidden="true">
                                    <?php if($message->file_type === 'pdf'): ?>
                                ...
                                <div class="file-info">
                                    <p class="file-name"><?php echo e($message->file_name); ?></p>
                                    <p class="file-meta"><?php echo e($message->file_type === 'pdf' ? 'PDF' : 'Dokumen'); ?> • <?php echo e(number_format($message->file_size / 1024, 1)); ?> KB</p>
                                </div>
                    ...
                    <p class="message-time <?php echo e($message->sender_id === auth()->id() ? 'text-right' : ''); ?>">
...
<style>
/* Messages Container - Hide Scrollbar */
#messages-container {
...
<script>
    // Setup Echo untuk LOCALHOST (HTTP)
    ...
    const chatApp = {
        ...
        setupWebSocket() {
            console.log('Setting up WebSocket for project:', this.projectId);
            
            const channel = window.Echo.channel('project.' + this.projectId);
            
            channel.subscription
                .bind('pusher:subscription_succeeded', () => {
                    console.log('✅ Subscribed to channel project.' + this.projectId);
                })
        ...
        setupForm() {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    const res = await fetch(`/projects/${this.projectId}/chat`, {
                        method: 'POST',
                        body: formData,
                        headers: {
        ...
        addMessage(data) {
            ...
            let fileHtml = '';
            if (data.file_path) {
                const url = `/chat/download/${data.id}`;
                if (data.file_type === 'image') {
                    fileHtml = `
                        <div class="file-preview-image">
                            <a href="${url}" target="_blank" rel="noopener noreferrer">
                                <img src="${url}" alt="${data.file_name}" loading="lazy">
                            </a>
                        </div>`;
                } else {
                    const iconClass = data.file_type === 'pdf' 
                        ? 'fa-file-pdf text-red-500' 
                        : 'fa-file-lines text-blue-500';
                    
                    fileHtml = `
                        <a href="${url}" class="file-download">
                            <div class="file-icon"><i class="fa-solid ${iconClass} text-2xl"></i></div>
                            <div class="file-info">
                                <p class="file-name">${data.file_name}</p>
                                <p class="file-meta">${data.file_type === 'pdf' ? 'PDF' : 'Dokumen'} • ${(data.file_size / 1024).toFixed(1)} KB</p>
                            </div>
            ...
            // Avatar HTML dengan photo_url
            const avatarHtml = isMe ? '' : `
                <div class="avatar">
                    <img src="${data.sender.photo_url || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(data.sender.name) + '&background=dc2626&color=fff&size=128'}" alt="${data.sender.name}">
                </div>
            `;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/ilyaskalamullah/Documents/SIP/sinom-jatimas/resources/views/chat/index.blade.php ENDPATH**/ ?>