...
<div class="space-y-6 animate-fade-in">
    ...
    <div class="bg-white p-6 rounded-[2.5rem] shadow-sm border border-gray-100"
        ...
        <form action="<?php echo e(route('admin.invoices.index')); ?>" method="GET" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 items-end">
            ...
            <div class="flex space-x-2 h-[46px]">
                ...
                <?php if(request()->anyFilled(['client_id', 'project_id', 'status'])): ?>
                    <a href="<?php echo e(route('admin.invoices.index')); ?>" class="w-14 inline-flex items-center justify-center bg-gray-100 text-gray-400 rounded-xl hover:bg-gray-200 transition-all shadow-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    <?php $__empty_1 = true; $__currentLoopData = $invoices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $invoice): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-gray-50/80 transition-all">
                            <td class="px-8 py-5">
                                <span class="font-black text-gray-900 tracking-tight"><?php echo e($invoice->invoice_number); ?></span>
                                <div class="text-[9px] text-gray-400 font-bold mt-1 uppercase">Diterbitkan: <?php echo e($invoice->created_at->format('d/m/Y')); ?></div>
                            </td>
                            <td class="px-8 py-5">
                                <div class="font-black text-gray-800 text-sm uppercase tracking-tight"><?php echo e($invoice->project->name); ?></div>
                                <div class="text-xs text-gray-500 font-medium mt-0.5"><?php echo e($invoice->project->client->name); ?></div>
                            </td>
                            ...
                            <td class="px-8 py-5">
                                <div class="flex items-center justify-center space-x-1">
                                    <a href="<?php echo e(route('admin.invoices.show', $invoice)); ?>" class="w-9 h-9 flex items-center justify-center text-gray-400 hover:text-gray-900 hover:bg-white hover:shadow-sm rounded-xl transition-all"><i class="fa-solid fa-eye text-xs"></i></a>
                                    <a href="<?php echo e(route('admin.invoices.download', $invoice)); ?>" class="w-9 h-9 flex items-center justify-center text-blue-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all"><i class="fa-solid fa-file-pdf text-xs"></i></a>
                                    
                                    <?php if(in_array($invoice->status, ['sent', 'overdue'])): ?>
                                        <form action="<?php echo e(route('admin.invoices.mark-paid', $invoice)); ?>" method="POST" class="inline" onsubmit="return confirm('Konfirmasi pelunasan invoice ini?')">
                                            <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/ilyaskalamullah/Documents/SIP/sinom-jatimas/resources/views/admin/invoices/index.blade.php ENDPATH**/ ?>